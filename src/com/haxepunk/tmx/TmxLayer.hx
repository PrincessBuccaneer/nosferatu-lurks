...
class TmxLayer
{
	...
	public function new(source:Fast, parent:TmxMap)
	{
		...
		if(data != null)
		{
			...
			if (data.has.encoding) {
					...
					if (data.has.compression)
					{
						switch(data.att.compression)
						{
							case "zlib":
								compressed = true;
							default:
								throw "TmxLayer - data compression type " + data.att.compression + "not supported!";
						}
					}
				...
				default:
					throw "TmxLayer - data encoding type " + data.att.encoding + " not supported!";
				}
			} else {
	...
	*/
	...
	private static function base64ToArray(chunk:String, lineWidth:Int, compressed:Bool):Array<Array<Int>>
	{
		...
		if (compressed) {
			#if html5
				throw "TmxLayer - no uncompress function on this platform!";
			#else
				data.uncompress();
			#end
		}
